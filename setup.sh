...
echo "!!! This will force remove $GO_DIR & $RESTIC_DIR directories, and reset GOPATH environment variable; in order to have a clean setup !!!"

read -p "Do you want to continue? (Y/n) " continue_1_answer
if [[ $continue_1_answer =~ ^[Nn] ]]; then
...
patch -p1 --verbose < index-max-age.2026-08-28.patch
...
echo "Setup should be successfull! You can now build using ./build-linux.sh and/or ./build-windows.sh"
